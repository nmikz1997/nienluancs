<?php
	if(empty($_GET['tentruyen']) || empty($_GET['tinhtrang']) ){header("location:trangchu.php");};
...
<?php 
	$tentruyen = mysqli_real_escape_string($conn,$_GET['tentruyen']);
	if(($_GET['tinhtrang'] != 'new') && ($_GET['tinhtrang'] != 'old')){
		$tinhtrang='new';
	}else{
		$tinhtrang = mysqli_real_escape_string($conn,$_GET['tinhtrang']);
	}
	
	$select = "SELECT truyen.matruyen,tentruyen,bia,gia
				from truyen
					join chitiettruyen on truyen.matruyen=chitiettruyen.matruyen
					where tentruyen LIKE '%$tentruyen%' and tinhtrang='$tinhtrang'";
...
<div class="container">
	<div class="row mt-5">
		<h3 class="list-product-title">Danh sách cần tìm</h3>
		<div class="product-group">
			<div class="row">
				<?php
					$num_rows=mysqli_num_rows($hienthi);
						if ($num_rows == 0) {
							echo "<h3>Trống</h3>";
						}else if ($num_rows == 1){
							$row = mysqli_fetch_assoc($hienthi);
							echo "<META http-equiv='refresh' content='0;URL=chitietTruyen.php?matruyen=".$row['matruyen']."&tinhtrang=".$tinhtrang."'>";
						}else if ($num_rows == 2){
							while ($row = mysqli_fetch_assoc($hienthi)){
								...
								<div class='card-header border-0'>
								  <img class='card-img-top' src=../Database/$tinhtrang/".$row["bia"]." height=240px width=200px alt='Card image cap'>
								</div>
								  <div class='card-body'>
								    <h6 class='card-title'>".$row["tentruyen"]."</h6>
								    <p class='card-text'>".number_format($row["gia"],0,',','.')." VNĐ</p>
								    <a href='chitietTruyen.php?matruyen=".$row['matruyen']."&tinhtrang=".$tinhtrang."' class='btn btn-primary'>Xem chi tiết</a>
								    <a href='addtocart.php?matruyen=".$row['matruyen']."&tinhtrang=".$tinhtrang."' class='btn btn-success' >Chọn mua</a>
								  </div>
								</div>
								</div>
								";
