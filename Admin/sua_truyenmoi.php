...
<head>
	<meta charset="utf-8">
	<title>Sửa chi tiết truyện mới</title>
...
<div>
	...
	<nav class="navbar navbar-default">
	  <div class="container-fluid">
	    ...
	    <ul class="nav navbar-nav">
	      ...
	      <li class="active"><a href="sua_truyenmoi.php">Sửa truyện</a></li>
	    </ul>
	  </div>
	</nav>
	<?php
		...
		from chitiettruyen
				...
				where truyen.matruyen='$matruyen' AND tinhtrang='new'";
		...
		if(isset($_POST['btnSua'])){
			...
			if($anh['type'] =="image/jpg" || $anh['type'] =="image/jpeg" || $anh['type'] =="image/png" || $anh['type'] =="image/gif")
			{
				if($anh['size'] <= 614400)
				{
					$tenanh =$matruyen."new_".$anh['name'];
					$update_truyenmoi= "update chitiettruyen 
										set
											gia			='".$_POST['gia']		."',
											soluong		='".$_POST['soluong']	."',
											bia			='".$tenanh				."'
										where matruyen	='".$_POST['matruyen']	."' and tinhtrang = 'new'
										";
					if($conn->query($update_truyenmoi))
					{	
						if (file_exists("../Database/New/".$bia))
							{
							    unlink("../Database/New/".$bia);
							}
						copy($anh['tmp_name'],"../Database/New/".$tenanh);
						echo "Đã Sửa truyện Mới";
						echo '<meta http-equiv="refresh" content="1;URL=sua_truyenmoi.php?matruyen='.$matruyen.'">';
					}
					else{
						echo "Lỗi: Sửa truyện mới thất bại";					
					}			
				}else{
	...
	?>
	<div class="container">
		<caption><h2>Sửa truyện Mới</h2></caption>
